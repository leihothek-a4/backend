...
class BluetoothManager(System, EventListener):
    def __init__(self) -> None:
        System.__init__(self, name="BluetoothManager")
    ...
    def on_event(self, event: NetworkEvent) -> None:
        ...
        if self._server is None:
            logger.warning(
                "Network disconnect - bluetooth service unreachable, queuing event for later."
            )
    ...
    def _send_event(self, event: NetworkEvent) -> None:
        try:
            ...
            logger.info(
                f"Network disconnect forwarded to bluetooth service: {event.prev_state} -> {event.new_state}"
            )
    ...
    def _flush_pending_events(self) -> None:
        if not self._pending_events:
            return
        logger.info(
            f"Bluetooth service reconnected - flushing {len(self._pending_events)} pending event(s)."
        )
    ...
    def _acquire_server(self) -> bool:
        try:
            ...
            logger.debug("Bluetooth service proxy acquired.")
            return True
        except dbus.DBusException as e:
            logger.warning(f"Bluetooth service not reachable yet: {e}")
            self._server = None
            return False
    ...
    def _attempt_connect(self) -> None:
        try:
            if not bool(self._server.HasDevice()):
                logger.debug("Service is still scanning - waiting.")
                return
            ...
            logger.error(f"DBus error: {e} - dropping service proxy, will re-acquire.")
            self._server = None
    ...
    def _subscribe_signals(self) -> None:
        ...
        logger.debug("Subscribed to bluetooth service signals.")
